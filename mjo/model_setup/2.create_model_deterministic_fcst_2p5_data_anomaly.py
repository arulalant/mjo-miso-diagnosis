...
def createDeterministicFcstMJOanomalies(curday, nextday):
    ...
    updateCdmlFile(ncfilepath, 'obsolr_um_analysis_data.xml',
                   os.path.join(fcstdatapath,
                                'obsolr_um_analysis_tseries-determisitic_10days_fcst_%s_data.xml' % curday))
    # update cdscan xml daily by editing xml itself.
    os.chdir(anaanopath)
    updateCdmlFile(fcstanofpath, 'obsolr_um_analysis_anomaly.xml',
                   os.path.join(fcstanopath, 'obsolr_um_analysis_tseries-determisitic_10days_fcst_%s_ano.xml' % curday))
...
if __name__ == '__main__':
    ...
    while pDay <= tDay:  # if we use <= it will run for the actual current day next 10 days fcst.
        ...
        outfile = os.path.join(fcstanopath, 'obsolr_um_analysis_tseries-determisitic_10days_fcst_%s_ano.xml' % pastDay)
