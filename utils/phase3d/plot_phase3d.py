...
def makeMjoPhases3DPlots(rawOrAnomaly,
                         filteredOrNot,
                         year,
                         yearDir=None,
                         seasons=['sum', 'win'],
                         seasonDir='All',
                         pcs1VarName='norm_pcs1',
                         pcs2VarName='norm_pcs2',
                         pcs2Sign=-1,
                         cyclic=False,
                         **kwarg):
    """
    ...
    inpath = os.path.join(processfilesPath, 'Level2', 'Ceof', rawOrAnomaly, filteredOrNot, yearDir)
    opath = os.path.join(plotsgraphsPath, 'Level2', 'Phase3D', rawOrAnomaly, filteredOrNot, str(year))  ###???
    for subName in os.listdir(inpath):
        ...
        infile = 'ceof_vars_%s_%s_%s_%s_%s.nc' % (seasonDir, yearDir, rawOrAnomaly, filteredOrNot, subName)
        ...
        ampfile = 'amppha_of_pcs_1_2_of_ceof_%s_%s_%s_%s_%s.nc' % (seasonDir, yearDir, rawOrAnomaly, filteredOrNot,
                                                                   subName)
        ...
        for season in seasons:
            ...
            if season == 'all':
                ...
                npc1 = timeobj.getWinterData(pcs1VarName, ceof_file, year=year, cyclic=cyclic)
                npc2 = timeobj.getWinterData(pcs2VarName, ceof_file, year=year, cyclic=cyclic)
            else:
                raise ValueError("arg 'season' must be either 'all/sum/win' only")
            # end of if season == 'all':
            outfile = 'Phase3D_%s_%s_%s_%s_%s' % (seasonName, yearDir, rawOrAnomaly, filteredOrNot, subName)
            ptitle = 'Phase3D %s %s %s %s' % (subName, seasonName, rawOrAnomaly, filteredOrNot)
...
if __name__ == '__main__':
    ...
    yearDir = '1979_2005'
    year = 1979
    makeMjoPhases3DPlots('Anomaly', 'Filtered', year=year, yearDir=yearDir, seasons=['sum', 'win'])
    ...
    makeMjoPhases3DPlots('Anomaly', 'Filtered', year=year, seasons=['win'], cyclic=True, timeorder=[11, 12, 1, 2, 3, 4])
