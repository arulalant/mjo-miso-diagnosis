...
def phase3d(xdata,
    ...
    Inputs:
        ...
        xdata1, ydata1 : forecast xaxis, yaxis dataset
    ...
    credits = "Created by using UVCDAT at NCMRWF "
    credits += "   Version : 0.1"
    ...
    x.Graph[0].yaxis.tick.place = 'both'
    ...
    x.Graph[0].xaxis.tick.place = 'both'
    # Now let's set the tick marks for Graph 0
    ...
    x.Graph[0].Set[csetno].symbol.size = 8
    ...
    if ptype == 'single':
        ...
        if xtimeAxis[:].tolist() != ytimeAxis[:].tolist():
            raise ValueError("xdata (%s), ydata (%s) time axis are not same!" % (xdata.id, ydata.id))
        ...
        avlColors = avlColors[:len(conMonths)]
        ...
        for montime, colr in zip(conMonths, avlColors):
            ...
            x.add_string(monx, 0.09, mon[:3], color=colr, char_size=0.6, rot=0, just=14)
            # increment the month string position
            monx += 0.05
            days1 = [date.day for date in pc1.getTime().asComponentTime()]
            if not days:
                days.extend(days1)
            else:
                if days[-1] == days1[0]:
        ...
        if xdata1.any() and ydata1.any():
            ...
            # for this month dataset
            x.Graph[0].Set[csetno].line.linewidth = 1
            x.Graph[0].Set[csetno].line.color = colors[-1]
            x.Graph[0].Set[csetno].line.linestyle = 'solid'
            x.Graph[0].Set[csetno].symbol.type = 1
            x.Graph[0].Set[csetno].symbol.size = 0.125
            x.Graph[0].Set[csetno].symbol.color = 'black'
            # plotting the xdata, ydata in circle manner
            x.plot(ydata1, xs=xdata1, G=0, S=csetno)
            # plot the little big circle at begining of forecast
            csetno = csetno + 1
            x.add_set(graph=0)
            x.Graph[0].Set[csetno].symbol.type = 1  # 'circle' type
            x.Graph[0].Set[csetno].symbol.size = 0.2  # just big circle indicates begining of forecast
            x.Graph[0].Set[csetno].symbol.color = 'black'
            x.Graph[0].Set[csetno].symbol.fcolor = 'black'
            x.plot([[ydata1[0]]], [[xdata1[0]]], G=0, S=csetno)
            days1 = [date.day for date in xdata1.getTime().asComponentTime()]
            if days[-1] == days1[0]:
                days[-1] = ''
                days.extend(days1)
            else:
    ...
    x.add_string(start_x - 0.2, start_y - 0.2, 'START', color=start_clr, char_size=0.6, rot=0, just=14)
    x.String[-1].loctype = 'world'
    print days, len(days), len(list(xdata) + list(xdata1))
    for dx, dy, day in zip(list(xdata) + list(xdata1), list(ydata) + list(ydata1), days):
        x.add_string(dx - 0.01, dy - 0.1, str(day), color='black', char_size=0.4, rot=0, just=14)
        x.String[-1].loctype = 'world'

    # draw the xdata id as xaxis label
    x.add_string(0.53, 0.16, xdata.id, color='black', char_size=0.7, rot=0, just=14)
    # draw the ydata id as yaxis label
    x.add_string(0.10, 0.48, ydata.id, color='black', char_size=0.7, rot=90, just=14)
    # draw the bottom comment
    x.add_string(0.53, 0.13, bcomment, color='black', char_size=1.3, rot=0, just=14)
    # draw the top comment
    x.add_string(0.53, 0.81, tcomment, color='black', char_size=1.3, rot=0, just=14)
    # draw the left comment
    x.add_string(0.06, 0.48, lcomment, color='black', char_size=1.3, rot=90, just=14)
    # draw the right comment
    x.add_string(0.97, 0.48, rcomment, color='black', char_size=1.3, rot=270, just=14)

    if dmax <= 2:
        # phase quadrant/positions no, and its x, y coordinate points to draw
        # phase name inside the graph
        phases_loc_in = ((1, (1.25, 0.8)), (2, (0.75, 2.15)), (3, (-1.25, 2.15)), (4, (-1.75, 0.8)), (5, (-1.75, -1)),
                         (6, (-1.25, -2.35)), (7, (0.65, -2.35)), (8, (1.25, -1)))
        # phase quadrant/positions no, its x, y coordinate points to draw
        # phase name and its rotation(direction) outside the graph
        phases_loc_out = ((1, (2.25, 1.75), 270), (2, (0.75, 2.25), 0), (3, (-1.25, 2.25), 0), (4, (-2.25, 1), 90),
                          (5, (-2.25, -1.5), 90), (6, (-1.25, -2.4), 0), (7, (0.65, -2.4), 0), (8, (2.25, -1.25), 270))

    elif dmax <= 3:
        # phase quadrant/positions no, and its x, y coordinate points to draw
        # phase name inside the graph
        phases_loc_in = ((1, (2.1, 1)), (2, (1, 3.25)), (3, (-2, 3.25)), (4, (-2.6, 1)), (5, (-2.6, -1)),
                         (6, (-2, -3.5)), (7, (1, -3.5)), (8, (2.1, -1)))
        # phase quadrant/positions no, its x, y coordinate points to draw
        # phase name and its rotation(direction) outside the graph
        phases_loc_out = ((1, (3.25, 2.5), 270), (2, (1, 3.25), 0), (3, (-2, 3.25), 0), (4, (-3.35, 1.5), 90),
                          (5, (-3.35, -2.5), 90), (6, (-2, -3.65), 0), (7, (1, -3.65), 0), (8, (3.25, -1.5), 270))
    elif dmax >= 4:
        # phase quadrant/positions no, and its x, y coordinate points to draw
        # phase name inside the graph
        phases_loc_in = ((1, (2.5, 1)), (2, (1, 3.5)), (3, (-2, 3.5)), (4, (-3.5, 1)), (5, (-3.5, -1)), (6, (-2, -3.5)),
                         (7, (1, -3.5)), (8, (2.5, -1)))
        # phase quadrant/positions no, its x, y coordinate points to draw
        # phase name and its rotation(direction) outside the graph
        phases_loc_out = ((1, (4.25, 2.5), 270), (2, (1, 4.25), 0), (3, (-2, 4.25), 0), (4, (-4.5, 1.5), 90),
                          (5, (-4.5, -2.5), 90), (6, (-2, -4.65), 0), (7, (1, -4.65), 0), (8, (4.25, -1.5), 270))
    else:
    ...
    if plocation in ['in', 'inside']:
        for (idx, (px, py)) in phases_loc_in:
            ...
            x.add_string(px, py, pname, color='black', char_size=0.8, rot=0, just=0)
        ...
        for (idx, (px, py), r) in phases_loc_out:
            ...
            x.add_string(px, py, pname, color='black', char_size=0.8, rot=r, just=0)
            x.String[-1].loctype = 'world'
...
def mjo_phase3d(npc1,
                ...
                tcomment='Western Pacific',
                bcomment='Indian Ocean',
                lcomment='Western Hemisphere & Africa',
                rcomment='Maritime Continent',
                title='MJO Phase diagram of PC-1 and PC-2',
                stitle1='',
                stitle2='',
                ctitle='Weak MJO',
                pposition1=5,
...
def miso_phase3d(xdata,
                 ydata,
                 sxyphase=None,
                 dmin=None,
                 dmax=None,
                 colors=['violet', 'green', 'orange', 'red'],
                 tcomment='Centre India & North India',
                 bcomment='Southern tp & IO',
                 lcomment='Peninsular & Centre India',
                 rcomment='FootHill & SIO',
